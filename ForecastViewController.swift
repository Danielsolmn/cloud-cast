...
class ForecastViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    private func changeLocation(withLocationIndex locationIndex: Int) {
        let location = locations[locationIndex] // get the selected location
        locationLabel.text = location.name 
        WeatherForecastService.fetchForecast(latitude: location.latitude,
                                             longitude: location.longitude) { forecast in
            self.configure(with: forecast) // update the UI
        }
    }

    private func configure(with forecast: CurrentWeatherForecast) {       // think CurrentWeatherForecast as 
